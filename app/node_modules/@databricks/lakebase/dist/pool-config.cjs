...
const require_token_refresh = require('./token-refresh.cjs');
...
function getLakebasePgConfig(config, telemetry, logger) {
	...
	const poolConfig = require_config.parsePoolConfig(userConfig);
	const username = require_config.getUsernameSync(userConfig);
	let passwordConfig;
	if (userConfig.password !== void 0) passwordConfig = userConfig.password;
	else if (poolConfig.endpoint) passwordConfig = require_token_refresh.createTokenRefreshCallback({
		userConfig,
		endpoint: poolConfig.endpoint,
		telemetry: telemetry ?? require_telemetry.initTelemetry(),
		logger
	});
	return {
...
//#endregion
exports.getLakebaseOrmConfig = getLakebaseOrmConfig;
exports.getLakebasePgConfig = getLakebasePgConfig;