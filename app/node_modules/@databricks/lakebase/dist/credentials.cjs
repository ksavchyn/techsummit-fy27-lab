...
function validateCredentialResponse(response) {
	if (typeof response !== "object" || response === null || !("token" in response) || !("expire_time" in response)) throw require_errors.ValidationError.invalidValue("credential response", response, "an object with { token, expire_time }");
	const { token, expire_time } = response;
	if (typeof token !== "string" || typeof expire_time !== "string") throw require_errors.ValidationError.invalidValue("credential response fields", {
...
//#endregion
exports.generateDatabaseCredential = generateDatabaseCredential;