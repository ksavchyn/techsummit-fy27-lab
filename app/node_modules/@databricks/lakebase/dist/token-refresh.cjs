...
const require_credentials = require('./credentials.cjs');
...
async function refreshToken(workspaceClient, endpoint) {
	const credential = await require_credentials.generateDatabaseCredential(workspaceClient, { endpoint });
	return {
...
function createTokenRefreshCallback(deps) {
	...
	return async () => {
		if (!workspaceClient) try {
			workspaceClient = require_config.getWorkspaceClient(deps.userConfig);
		} catch (error) {
...
//#endregion
exports.createTokenRefreshCallback = createTokenRefreshCallback;