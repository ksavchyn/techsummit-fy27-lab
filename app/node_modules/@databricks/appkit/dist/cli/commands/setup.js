...
function findInstalledPackages() {
	...
	for (const pkg of PACKAGES) {
		...
		if (fs.existsSync(packagePath)) installed.push(pkg);
	}
	return installed;
}
/**
...
function updateContent(existingContent, packages) {
	...
	if (startIndex !== -1 && endIndex !== -1) {
		...
		const after = existingContent.substring(endIndex + 32);
...
function runSetup(options) {
	...
	const existingContent = fs.existsSync(claudePath) ? fs.readFileSync(claudePath, "utf-8") : null;
	let finalContent;
	let action;
	if (existingContent) {
