...
import { HostType, ConfigType } from "./HostType";
...
export type AuthType = "default" | "pat" | "basic" | "azure-cli" | "azure-msi" | "azure-client-secret" | "google-credentials" | "google-id" | "metadata-service" | "databricks-cli" | "oauth-m2m";
export type AttributeName = keyof Omit<ConfigOptions, "credentials" | "logger" | "env" | "loaders">;
export type ConfigWithHost = Config & {
    host: string;
};
export declare function isConfigWithHost(config: Config): config is ConfigWithHost;
export declare class Config {
    ...
    /** Loaders to load the config variables from, if user does not set them explicitly in the config object */
    loaders?: Loader[];
    ...
    accountId?: string;
    /** Workspace ID for workspace clients using unified hosts */
    workspaceId?: string;
    /** Experimental flag to mark a host as unified (serving both workspace and account APIs) */
     ...
     */
    private fetchOidcDiscovery;
    fetch(url: string, options: any): ReturnType<typeof fetch>;
}
export declare function isAnyAuthConfigured(cfg: Config): boolean;
export {};
