import * as http from "node:http";
import * as https from "node:https";
import { Context } from "./context";
import { Config } from "./config/Config";
type HttpMethod = "POST" | "GET" | "HEAD" | "DELETE" | "PATCH" | "PUT";
type SearchParamsPrimitive = string | number | boolean | null | undefined;
export type SearchParamsValue = SearchParamsPrimitive | Array<SearchParamsPrimitive> | SearchParams;
export interface SearchParams {
    [key: string]: SearchParamsValue;
}
export type ProductVersion = `${number}.${number}.${number}`;
export interface ClientOptions {
    agent?: http.Agent | https.Agent;
    product?: string;
    productVersion?: ProductVersion;
    userAgentExtra?: Record<string, string>;
}
export declare class ApiClient {
    readonly config: Config;
    private _agent;
    readonly product: string;
    readonly productVersion: ProductVersion;
    readonly userAgentExtra: Record<string, string>;
    constructor(config: Config, options?: ClientOptions);
    get host(): Promise<URL>;
    get accountId(): Promise<string | undefined>;
    getAgent(): Promise<http.Agent | https.Agent>;
    userAgent(): string;
    searchParams(query: SearchParams): string;
    request(options: {
        path: string;
        method: HttpMethod;
        query?: SearchParams;
        headers: Headers;
        responseHeaders?: string[];
        raw: boolean;
        payload?: any;
    }, context?: Context): Promise<unknown>;
}
export {};
//# sourceMappingURL=api-client.d.ts.map