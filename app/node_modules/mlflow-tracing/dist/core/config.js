...
function init(config) {
    ...
    if (!trackingUri) {
        throw new Error('An MLflow Tracking URI is required, please provide the trackingUri option to init, or set the MLFLOW_TRACKING_URI environment variable');
    }
    if (!experimentId) {
        throw new Error('An MLflow experiment ID is required, please provide the experimentId option to init, or set the MLFLOW_EXPERIMENT_ID environment variable');
    }
    ...
    const databricksConfigPath = config.databricksConfigPath ?? path_1.default.join(os_1.default.homedir(), '.databrickscfg');
    // Validate non-Databricks URIs
    if (!(0, auth_1.isDatabricksUri)(trackingUri) && !isValidHttpUri(trackingUri)) {
        throw new Error(`Invalid trackingUri: '${trackingUri}'. Must be a valid HTTP or HTTPS URL, or 'databricks' / 'databricks://<profile>'.`);
    }
    ...
    globalAuthProvider = (0, auth_1.createAuthProvider)({
        trackingUri,
        host: config.host,
...
function getAuthProvider() {
    if (!globalAuthProvider) {
        throw new Error('The MLflow Tracing client is not configured. Please call init() before using tracing functions.');
    }
