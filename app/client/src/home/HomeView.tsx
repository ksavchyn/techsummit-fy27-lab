...
const HERO = {
  name: 'Marcus Bell',
  role: 'EVP, Consumer & Small Business Banking',
};

const STORY = {
  headline: 'What is the next best action for this customer?',
  situation:
    'A competitor rate promotion is pulling our most valuable, longest-tenured customers toward the door. Large CDs are maturing within days, attrition risk scores are spiking, and $689K in balance is about to walk out. The relationship manager needs to act—now.',
  goal: 'Surface the at-risk customer, let the model rank the best retention play, draft outreach, and execute—with human approval.',
};

const STARTER_QUESTIONS = [
  'Why is CUST-0000214 at risk of leaving?',
  'What are my options to retain this customer?',
  'Rank the next best action using the model.',
];

const FEATURED_ACTION_PROMPT =
  'CUST-0000214 is at risk of leaving. Find out why, use the recommender model to rank my options, draft a retention offer, and show me before executing. Wait for my approval.';
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-8 py-6 sm:py-14 space-y-5 sm:space-y-7">
        ...
        <section className="space-y-5">
          <div className="hidden sm:block text-[11px] font-semibold uppercase tracking-[0.18em] text-muted-foreground">
            The decision loop · surface → prescribe → approve → act
          </div>
          ...
          >
            ...
            <div className="relative">
              ...
              <h3 className="display text-2xl font-semibold mb-2 leading-tight">
                Retain CUST-0000214 — let the model pick the best play
              </h3>
              <p className="hidden sm:block text-sm opacity-85 leading-relaxed mb-5 max-w-2xl">
                The assistant investigates why the customer is at risk, then asks the
